...
public class CategoriaController : MonoBehaviour {
	...
	public void showInstructions(int categoria){
		if(Util.getLevel()>0){
			...
			if(Util.getLevel()==1){
				Util.setMessage("Select the image that matches to spoken word and appears in the top box");
				...
				Util.setMessage("Touch an image and then touch the word that match with the selected image");
				Util.setMensaje("Toca una imagen y luego toca la palabra que corresponda a la imagen seleccionada");
			}else{
				Util.setMessage("Look the images that drop and touch the image that correspond to spoken word and that appears in the green box");
