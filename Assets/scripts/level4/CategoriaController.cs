...
public class CategoriaController : MonoBehaviour {
	...
	public void showInstructions(int categoria){
		if(Util.getLevel()>0){
			...
			if(Util.getLevel()==1){
				Util.setMessage("Select the image that matches to the spoken word and that appears in the top box");
				...
				Util.setMessage("Look at the images that drop and touch the image that corresponds to the spoken word and that appears in the green box");
